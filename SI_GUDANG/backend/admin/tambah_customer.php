...
$last = mysqli_fetch_assoc(mysqli_query($conn, "SELECT id_customer FROM tb_customer ORDER BY id_customer DESC LIMIT 1"));
if($last){
...
if(isset($_POST['submit'])){
    ...
    $cek = mysqli_query($conn, "SELECT * FROM tb_customer WHERE id_customer='$id_customer'");
    if(mysqli_num_rows($cek) > 0){
        echo "<script>alert('ID Customer sudah ada!');</script>";
    } else {
        mysqli_query($conn, "INSERT INTO tb_customer (id_customer, nama_customer, jenis_kelamin, alamat_customer, telepon_customer, email_customer, pass_customer)
