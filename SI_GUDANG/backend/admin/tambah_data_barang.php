...
$last = mysqli_fetch_assoc(mysqli_query($conn, "SELECT kd_barang FROM tb_barang ORDER BY kd_barang DESC LIMIT 1"));
if($last){
...
if(isset($_POST['submit'])){
    ...
    $cek = mysqli_query($conn, "SELECT * FROM tb_barang WHERE kd_barang='$kd_barang'");
    if(mysqli_num_rows($cek) > 0){
        echo "<script>alert('Kode barang sudah ada!');</script>";
    } else {
        mysqli_query($conn, "INSERT INTO tb_barang (kd_barang, kode_jenis, nama_barang, stok, harga_beli, harga_jual, gambar_produk)
